...
print("Fetching all the matches provided by cricbuzz: ")
print(json.dumps(matches, indent = 4))


mid = input("Enter match_id of currently available matches from cricbuzz: ")
minfo = c.matchinfo(mid)
lscore = c.livescore(mid)
comm = c.commentary(mid)
scard = c.scorecard(mid)

print("Fetching the match information: ")
print(json.dumps(minfo, indent=4, sort_keys=True))

print("Fetching the live score of a match: ")
print(json.dumps(lscore, indent=4, sort_keys=True))

print("Fetching commentary of the match: ")
print(json.dumps(comm, indent=4, sort_keys=True))

print("Fetching scorecard of a match: ")
print(json.dumps(scard, indent=4, sort_keys=True))

with open('cricbuzz.txt', 'w') as f:
        json.dump(matches, f, ensure_ascii = False)
        f.write("\n\n")
        json.dump(minfo, f, ensure_ascii = False)
        f.write("\n\n")
        json.dump(lscore, f, ensure_ascii = False)
        f.write("\n\n")
        json.dump(comm, f, ensure_ascii = False)
        f.write("\n\n")
        json.dump(scard, f, ensure_ascii = False)

print("Written the json data to .txt file")
